...
MainWindow::MainWindow(QWidget *parent) :
   ...
   //setup DDE
   DDEConnected  = false;
   DDEAPP = QString::fromUtf8("Orbitron");
   DDETOPIC = QString::fromUtf8("Tracking");
   DDEITEM = QString::fromUtf8("TrackingData");
   DDEConversation = DDEPort.open(DDEAPP, DDETOPIC);

   //connect the TCP/IP signals!
   ...
   connect(ui->pushButton_DDE, SIGNAL(clicked()), this, SLOT(toggleDDEConnection()));
...
void MainWindow::toggleActiveTracking()
   ...
   if(ActiveTrack == true) //we're tracking, so stop it
      {
      ActiveTrack = false;
      ui->pushButton_GO->setText("Track");
...
void MainWindow::toggleDDEConnection()
   {
   if(DDEConnected == false)
      {
      ...
      if(DDEConversation)
         {
         DDEConnected = true;
         ui->pushButton_DDE->setText("Pause DDE");
         statusBar()->showMessage(tr("Connected to Orbitron DDE"),10000);
      ...
      {
      //DDEPort.close(DDEConversation);
      DDEConnected = false;
      ui->pushButton_DDE->setText("Connect DDE");
      statusBar()->showMessage(tr("Paused Orbitron DDE"),10000);
...
void MainWindow::parseDDEIncoming()
   {
   //examples from Orbitron DDE
   //"SNJY1SAT_(JO-97) AZ340.0 EL1.0 DN145831705 UP145003276"
   //"SNITASAT_1 AZ198.4 EL6.4 DN145862998 UP144997019"
   //"SNNOAA_18 AZ182.5 EL-36.7 DN137352303 UP144997569"
   //"SNNOAA_18 AZ273.3 EL7.8 DN137349818 UP145000192 DMFM"
   //if(!DDEConnected)

   if(!DDEConversation)
      return;

   QRegularExpression RegAz("^SN(.*) AZ(-?\\d+\\.?\\d*) EL(-?\\d+\\.?\\d*)\\W*");

   QString DDELineIn = DDEPort.requestPersistant(DDEConversation, DDEAPP, DDETOPIC, DDEITEM);
   //qDebug() << SerLineIn;
   QRegularExpressionMatch match(RegAz.match(DDELineIn));
   if (match.hasMatch())
      {
      currentDDETarget = match.captured(1);
      DDETargetAz = match.captured(2).toDouble();
      DDETargetEl = match.captured(3).toDouble();
      if(DDEConnected)
         {
         CommandedRotorAz = DDETargetAz;
         CommandedRotorEl = DDETargetEl;
         }
      ...
      //ui->lineEdit_DDEPort->setText(currentDDETarget);
      ui->lineEdit_DDEPort->setText(QString("%1 : AZ %2 EL %3").arg(currentDDETarget, match.captured(2), match.captured(3)));
